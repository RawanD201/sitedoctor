...
if(! function_exists('generate_report_card')){
    function generate_report_card($class="",$item="",$status="",$value="",$short_recommendation="",$long_recommendation="",$type="",$data="")
    {
        $value = ($value!="" & is_numeric($value)) ? "<b class='h4'>".$value."</b>": $value;
        if($type=="card") {
            $content = '<div class="card card-'.$class.' min-h-400px">
                        <div class="card-header">
                            <h4 class="text-'.$class.'"><i class="fas fa-'.$status.' text-'.$class.'"></i> '. $item.'</h4>
                        </div>

                        <div class="card-body pt-3">
                            <div class="card-text mb-3">'.$value.'</div>';
                            if($short_recommendation!="") {
                                $content .= '<h4 class=""><i class="fas fa-vote-yea"></i> '.__("Short Recommendation").'</h4>';
                                $content .= '<p class="card-text">'.$short_recommendation.'</p>';
                            }
                            $content .= '<div>
                                <h4 class=""><i class="fas fa-book"></i> '.__("Knowledge Base").'</h4>

                                <div class="alert alert-secondary border-0">
                                    <p>'.$long_recommendation.'</p>
                                </div>
                            </div>
                        </div>
            ...
            $content = '<div class="col-12 col-md-6">
                            <div class="card card-'.$class.' min-h-400px">
                                <div class="card-header">
                                    <h4 class="text-'.$class.'"><i class="fas fa-'.$status.' text-'.$class.'"></i> '. $item.'</h4>
                                </div>
                                        ...
                                        <table class="table table-bordered table-striped text-center">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th>'.__("Occurrence").'</th>
                                                    <th>'.__("Density").'</th>
                                                    <th>'.__("Possible Spam").'</th>
                                                </tr>
                                            </thead>
                                            <tbody>';
                                            foreach ($data[2] as $key => $valuex) {
                                                $content .= '<tr>
                                                        <td>'.$key.'</td>
                                                        <td>'.$valuex.'</td>
                                                        <td>'.$occurence = round((($valuex/$data[0])*100),3).' %</td>
                                                        <td>';
                                                        if(in_array(strtolower($key), $data[1]))
                                                            $content .=  "Yes";
                                                        else
                                                          $content .= 'No';
                                                         $content .=  '</td>
...
if ( ! function_exists('set_email_config'))
{
    function set_email_config(){
        ...
        config(['app.name' => config('settings.product_name')]);
        config(['mail.from.name' => config('settings.product_name')]);
        config(['mail.default' => 'sendmail']);
        config(['mail.from.address' =>  'no-reply@'.$agent_domain]);
        $getActiveEmailConfig = DB::table("email_config")->where("status","1")->get();
        if($getActiveEmailConfig->isEmpty()) return false;

        if(isset($getActiveEmailConfig[0])) {
            if($getActiveEmailConfig[0]->encryption=='Default') $getActiveEmailConfig[0]->encryption = 'tls';            
            config([
                'mail.default' =>'smtp',
                'mail.from.address' => $getActiveEmailConfig[0]->email_address ??  '',
                'mail.mailers.smtp.host' => $getActiveEmailConfig[0]->smtp_host ?? '',
                'mail.mailers.smtp.port' => $getActiveEmailConfig[0]->smtp_port ?? '',
                'mail.mailers.smtp.encryption' => $getActiveEmailConfig[0]->encryption ?? 'tls',
                'mail.mailers.smtp.username' => $getActiveEmailConfig[0]->smtp_user ?? '',
                'mail.mailers.smtp.password' => $getActiveEmailConfig[0]->smtp_password  ?? ''
            ]);
        }
        return true;
    }
...
{
  function convert_datetime_to_phrase($input_datetime, $format=false,$current_datetime=null)
  {
...
{
    // which_price'=>'both','original','sale'
...
if ( ! function_exists('get_marcadopago_country_list')) {
    function get_marcadopago_country_list()
    {
...
if ( ! function_exists('has_module_action_access')) {
    function has_module_action_access($module_id=0,$action=null,$team_access=[],$is_manager=false){
        if(!$is_manager) return true;
        if(empty($action)) $action = [1,2,3,4];
        if(empty($team_access) || !isset($team_access[$module_id])) return false;
        if(!is_array($action)) $action = [$action];
        $found=false;
        foreach ($action as $k=>$v){
            if(in_array($v,$team_access[$module_id])) $found=true;
        }
        return $found;
    }
}
...
if(!function_exists('run_curl')){
    ...
    {
        //$is_telegram_api = false;
        //if (strpos($url, 'api.telegram.org') !== false) $is_telegram_api = true;
...
if ( ! function_exists('bot_condition')) {
    function bot_condition($variable, $value, $operator)
    {
        switch ($operator) {
...
if(! function_exists('get_selected_sidebar')){
    function get_selected_sidebar($route='dashboard')
    {
        $map_list = [
            ...
            'lead' => ['settings.lead.lists'],
            'site-health-report' => ['domain.site.health'],
            'comparative-health-report' => ['domain.comaparative.site.health'],
        ];
        $select = $route;
...
if(! function_exists('full_width_page_routes')){
    function full_width_page_routes(){
        return ['bot-manager','whatsapp-bot-manager','ecommerce-stores','ecommerce-stores-lists','livechat-load','whatsapp-livechat-load'];
    }
}

if(! function_exists('display_landing_content')){
    function display_landing_content($string=''){
        return str_replace(['{{APP_NAME}}','{{BOT_FATHER}}'],[config('settings.product_name'),'<a href="https://t.me/botfather" target="_BLANK">BotFather</a>'],$string);
    }
}
...
if ( ! function_exists('get_public_path'))
{
    function get_public_path($param='') {
        $replace = get_domain_only(ENV('APP_URL'))=='sitedoctor.test' ? 'public' : 'html';
...
if(! function_exists('get_whatsapp_api_error_message')){
    function get_whatsapp_api_error_message($response=[]){
...
//DO NOT DELETE THIS, USED BY MULTILINGUAL EDITOR PACKAGE
