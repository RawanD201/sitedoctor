...
class Multilanguage extends Home
{
	...
	private $translation;
	public $defaultFiles=['auth.php','pagination.php','passwords.php','validation.php','ticket.php'];
	public $vendor_directories;
	public $exclude_groups = ['ticket.php']; // agent users dont need this
	...
	public function delete($language=null)
	{
        ...
        $flowbuilder_file = storage_path('app'.DIRECTORY_SEPARATOR.'public'.DIRECTORY_SEPARATOR.'lang'.DIRECTORY_SEPARATOR.'flowbuilder'.DIRECTORY_SEPARATOR.$locale_name.'.js');
		...
		@File::delete($flowbuilder_file);

        $server_ips = explode(',',ENV('SERVER_IP'));
        if(!empty($server_ips)){
            //$errors = array();
            foreach ($server_ips as $ip){
                if(empty($ip)) continue;
                $curl_url = 'http://'.$ip.'/cron/language/delete/'.ENV('CRON_TOKEN');
                $post_data = ['lang'=>$language];
                $response = run_curl($curl_url,$post_data);
        ...
        foreach($fileList as $file) {
            ...
            foreach ($currentFile as $k=>$v){
                if(!isset($enCurrentFile[$k]) && isset($currentFile[$k])) unset($currentFile[$k]);
                    ...
                    if($fileNameOnly=='flowbuilder'){
                        $js_flowbuilder .= 'var '.$k.' = "'.$v.'";'.PHP_EOL;
                    }
                }
            }

            // write flowbuilder js file
            if($fileNameOnly=='flowbuilder'){
                $flowbuilder_file = storage_path('app'.DIRECTORY_SEPARATOR.'public'.DIRECTORY_SEPARATOR.'lang'.DIRECTORY_SEPARATOR.'flowbuilder'.DIRECTORY_SEPARATOR.$language.'.js');
                file_put_contents($flowbuilder_file,$js_flowbuilder);
            }
